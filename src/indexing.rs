    ...
    stream::iter(functions.chunks(100).enumerate())
        .map(|(i, chunk)| {
            ...
            async move {
                ...
                let result = neo
                    .run(query(&query_text).param("functions", functions_json))
                    .await;
...
fn process_file(
    ...
    for import in root.descendants().filter(|n| n.kind() == "use_declaration") {
    ...
    for fn_node in root.descendants().filter(|n| n.kind() == "function_item") {
